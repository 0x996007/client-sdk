...
import { toTimestamp, fromTimestamp, DeepPartial } from '../../helpers.js';
...
export const AllDowntimeInfo = {
  encode(
    ...
    for (const v of message.infos) {
      AllDowntimeInfo_DowntimeInfo.encode(
        v!,
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): AllDowntimeInfo {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          message.infos.push(
            AllDowntimeInfo_DowntimeInfo.decode(reader, reader.uint32()),
          );
          break;
...
export const AllDowntimeInfo_DowntimeInfo = {
  ...
  fromPartial(
    object: DeepPartial<AllDowntimeInfo_DowntimeInfo>,
  ): AllDowntimeInfo_DowntimeInfo {
    const message = createBaseAllDowntimeInfo_DowntimeInfo();
    message.duration =
