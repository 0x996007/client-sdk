...
import {
  PerpetualPosition,
  PerpetualPositionSDKType,
} from './perpetual_position.js';
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): Subaccount {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 2:
          message.assetPositions.push(
            AssetPosition.decode(reader, reader.uint32()),
          );
          break;

        case 3:
          message.perpetualPositions.push(
            PerpetualPosition.decode(reader, reader.uint32()),
          );
          break;
