...
export const EpochStats = {
  encode(
    ...
    if (message.epochEndTime !== undefined) {
      Timestamp.encode(
        toTimestamp(message.epochEndTime),
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): EpochStats {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          message.epochEndTime = fromTimestamp(
        ...
        case 2:
          message.stats.push(
            EpochStats_UserWithStats.decode(reader, reader.uint32()),
          );
          break;
...
export const EpochStats_UserWithStats = {
  encode(
    message: EpochStats_UserWithStats,
    writer: _m0.Writer = _m0.Writer.create(),
  ): _m0.Writer {
    if (message.user !== '') {
  ...
  fromPartial(
    object: DeepPartial<EpochStats_UserWithStats>,
  ): EpochStats_UserWithStats {
    const message = createBaseEpochStats_UserWithStats();
