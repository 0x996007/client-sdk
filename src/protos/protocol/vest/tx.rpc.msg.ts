...
export interface Msg {
  ...
  deleteVestEntry(
    request: MsgDeleteVestEntry,
  ): Promise<MsgDeleteVestEntryResponse>;
}
export class MsgClientImpl implements Msg {
  ...
  setVestEntry(request: MsgSetVestEntry): Promise<MsgSetVestEntryResponse> {
    ...
    const promise = this.rpc.request(
      'dydxprotocol.vest.Msg',
      'SetVestEntry',
      data,
    );
    return promise.then((data) =>
      MsgSetVestEntryResponse.decode(new _m0.Reader(data)),
    );
  }
  ...
  deleteVestEntry(
    ...
    return promise.then((data) =>
      MsgDeleteVestEntryResponse.decode(new _m0.Reader(data)),
    );
  }
}
