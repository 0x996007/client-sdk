...
export const EquityTierLimitConfiguration = {
  ...
  decode(
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          message.shortTermOrderEquityTiers.push(
            EquityTierLimit.decode(reader, reader.uint32()),
          );
          break;

        case 2:
          message.statefulOrderEquityTiers.push(
            EquityTierLimit.decode(reader, reader.uint32()),
          );
          break;
  ...
  fromPartial(
    object: DeepPartial<EquityTierLimitConfiguration>,
  ): EquityTierLimitConfiguration {
    const message = createBaseEquityTierLimitConfiguration();
    message.shortTermOrderEquityTiers =
      object.shortTermOrderEquityTiers?.map((e) =>
        EquityTierLimit.fromPartial(e),
      ) || [];
    message.statefulOrderEquityTiers =
      object.statefulOrderEquityTiers?.map((e) =>
        EquityTierLimit.fromPartial(e),
      ) || [];
    return message;
  },
