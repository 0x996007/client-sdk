...
export interface Query {
  ...
  perpetualFeeParams(
    request?: QueryPerpetualFeeParamsRequest,
  ): Promise<QueryPerpetualFeeParamsResponse>;
  /** Queries a user's fee tier */

  userFeeTier(
    request: QueryUserFeeTierRequest,
  ): Promise<QueryUserFeeTierResponse>;
}
export class QueryClientImpl implements Query {
  ...
  perpetualFeeParams(
    ...
    const promise = this.rpc.request(
      'dydxprotocol.feetiers.Query',
      'PerpetualFeeParams',
      data,
    );
    return promise.then((data) =>
      QueryPerpetualFeeParamsResponse.decode(new _m0.Reader(data)),
    );
  }
  ...
  userFeeTier(
    ...
    return promise.then((data) =>
      QueryUserFeeTierResponse.decode(new _m0.Reader(data)),
    );
  }
}
