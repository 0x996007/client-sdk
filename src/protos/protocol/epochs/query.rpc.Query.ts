...
export interface Query {
  ...
  epochInfoAll(
    request?: QueryAllEpochInfoRequest,
...
export class QueryClientImpl implements Query {
  ...
  epochInfo(
    request: QueryGetEpochInfoRequest,
  ): Promise<QueryEpochInfoResponse> {
    const data = QueryGetEpochInfoRequest.encode(request).finish();
    const promise = this.rpc.request(
      'dydxprotocol.epochs.Query',
      'EpochInfo',
      data,
    );
    return promise.then((data) =>
      QueryEpochInfoResponse.decode(new _m0.Reader(data)),
    );
  }
  ...
  epochInfoAll(
    ...
    return promise.then((data) =>
      QueryEpochInfoAllResponse.decode(new _m0.Reader(data)),
    );
  }
}
