...
export const LiquidateSubaccountsRequest = {
  ...
  decode(
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 2:
          message.liquidatableSubaccountIds.push(
            SubaccountId.decode(reader, reader.uint32()),
          );
          break;

        case 3:
          message.negativeTncSubaccountIds.push(
            SubaccountId.decode(reader, reader.uint32()),
          );
          break;

        case 4:
          message.subaccountOpenPositionInfo.push(
            SubaccountOpenPositionInfo.decode(reader, reader.uint32()),
          );
          break;
  ...
  fromPartial(
    object: DeepPartial<LiquidateSubaccountsRequest>,
  ): LiquidateSubaccountsRequest {
    const message = createBaseLiquidateSubaccountsRequest();
    message.blockHeight = object.blockHeight ?? 0;
    message.liquidatableSubaccountIds =
      object.liquidatableSubaccountIds?.map((e) =>
        SubaccountId.fromPartial(e),
      ) || [];
    message.negativeTncSubaccountIds =
      object.negativeTncSubaccountIds?.map((e) =>
        SubaccountId.fromPartial(e),
      ) || [];
    message.subaccountOpenPositionInfo =
      object.subaccountOpenPositionInfo?.map((e) =>
        SubaccountOpenPositionInfo.fromPartial(e),
      ) || [];
    return message;
  },
};
...
export const LiquidateSubaccountsResponse = {
  ...
  fromPartial(
    _: DeepPartial<LiquidateSubaccountsResponse>,
  ): LiquidateSubaccountsResponse {
    const message = createBaseLiquidateSubaccountsResponse();
    return message;
  },
