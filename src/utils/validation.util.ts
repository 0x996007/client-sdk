...
import { MAX_SUBACCOUNT_NUMBER, MAX_UINT_32 } from '../types.js';
import {
  OrderFlags,
  ICancelOrder,
  IPlaceOrder,
...
export function validatePlaceOrderMessage(
  ...
  if (
    !isStatefulOrder(order.orderFlags) &&
    !verifyGoodTilBlock(order.goodTilBlock)
  ) {
    return new UserError(
      `goodTilBlock: ${order.goodTilBlock} is not a valid uint32 or is 0`,
    );
  }
  if (
    isStatefulOrder(order.orderFlags) &&
...
export function validateBatchCancelOrderMessage(
  ...
  if (!verifyGoodTilBlock(orders.goodTilBlock)) {
    return new UserError(
      `goodTilBlock: ${orders.goodTilBlock} is not a valid uint32 or is 0`,
    );
  }
...
export function validateTransferMessage(
  ...
  const addressError: Error | undefined = verifyIsBech32(
