...
export function calculateVaultQuantums(size: number): bigint {
  return BigInt(
    BigNumber(size).times(1_000_000).toFixed(0, BigNumber.ROUND_FLOOR),
  );
}
...
export function calculateTimeInForce(
  ...
  switch (type) {
    ...
    case OrderType.TAKE_PROFIT_LIMIT:
      if (execution == null) {
        throw new Error(
          'execution must be set if order type is STOP_LIMIT or TAKE_PROFIT_LIMIT',
        );
      }
      switch (execution) {
    ...
    case OrderType.TAKE_PROFIT_MARKET:
      if (execution == null) {
        throw new Error(
          'execution must be set if order type is STOP_MARKET or TAKE_PROFIT_MARKET',
        );
      }
      switch (execution) {
        case OrderExecution.DEFAULT:
...
export function calculateConditionType(
  orderType: OrderType,
): Order_ConditionType {
  switch (orderType) {
    case OrderType.LIMIT:
