...
export class QueryExecutor {
  ...
  constructor(
    tendermintClient: WarpedTendermintClient,
  ...
  async getDelayedCompleteBridgeMessages(
    ...
    const requestData: Uint8Array = Uint8Array.from(
      BridgeModule.QueryDelayedCompleteBridgeMessagesRequest.encode({
        address,
      }).finish(),
    );
  ...
  async getWithdrawalAndTransferGatingStatus(
    ...
    return SubaccountsModule.QueryGetWithdrawalAndTransfersBlockedInfoResponse.decode(
      data,
    );
  }
  ...
  async getAffiliateInfo(
    address: string,
  ): Promise<AffiliateModule.AffiliateInfoResponse> {
    const requestData = Uint8Array.from(
  ...
  async getReferredBy(
    address: string,
  ): Promise<AffiliateModule.ReferredByResponse> {
    const requestData = Uint8Array.from(
